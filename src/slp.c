...
esp_err_t slp_send_bool(const char *key, bool value)
{
    ...
    cbor_encode_int(&map_encoder, context.device_id);
    ...
    {
        ESP_LOGI(TAG, "attempt %d/%d", i, context.retries);
        if (lora_send(buffer, length) != pdTRUE)
        {
            ESP_LOGE(TAG, "error sending message");
            continue;
        }
        ESP_LOGI(TAG, "waiting for ack");
        if (lora_receive(ack, 1, pdMS_TO_TICKS(context.receive_windows)) > 0 && ack[0] == context.device_id)
        {
